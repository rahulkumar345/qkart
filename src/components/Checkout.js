...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const Address_POST_API = `${config.endpoint}/user/addresses`;
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      const Address_Delete_API = `${config.endpoint}/user/addresses/${addressId}`;
      const response = await axios.delete(Address_Delete_API, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setAddresses({ ...addresses, all: response.data });
    } catch (e) {
  ...
  const validateRequest = (items, addresses) => {
    ...
    const payableCost = items.map((item) => item.qty * item.cost);
    const totalCost = payableCost.reduce((total, value) => total + value);
    ...
    if (addresses.all.length < 0) {
      ...
      return false;
    } else if (addresses.all.length > 0 && addresses.selected === "") {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      const Checkout_API_Post = `${config.endpoint}/cart/checkout`;
      const response = await axios.post(
        Checkout_API_Post,
